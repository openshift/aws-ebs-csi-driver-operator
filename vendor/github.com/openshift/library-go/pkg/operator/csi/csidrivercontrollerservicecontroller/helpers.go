...
func WithServingInfo() dc.ManifestHookFunc {
	return func(opSpec *opv1.OperatorSpec, manifest []byte) ([]byte, error) {
		...
		cipherSuites, cipherSuitesFound, err := unstructured.NestedStringSlice(config, "servingInfo", "cipherSuites")
		if err != nil {
			return nil, fmt.Errorf("couldn't get the servingInfo.cipherSuites config from observed config: %w", err)
		}

		minTLSVersion, minTLSVersionFound, err := unstructured.NestedString(config, "servingInfo", "minTLSVersion")
		if err != nil {
