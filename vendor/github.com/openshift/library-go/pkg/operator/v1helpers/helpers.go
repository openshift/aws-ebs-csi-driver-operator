...
func IsDeploymentProgressing(conditionName string, deployment *appsv1.Deployment, status *operatorv1.OperatorStatus) (bool, string) {
	...
	missingReplicas := expectedReplicas - deployment.Status.UpdatedReplicas
	if missingReplicas > 0 {
		return true, fmt.Sprintf("Waiting for Deployment to deploy node pods (%d)", missingReplicas)
	}
